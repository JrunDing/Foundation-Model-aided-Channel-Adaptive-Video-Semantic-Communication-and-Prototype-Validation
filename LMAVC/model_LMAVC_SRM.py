...
class FrameCodec(nn.Module):
    ...
    def forward(self, referframe, input_image, H_f):
        ...
        semantic_feature, semantic_feature_shape = self.videocompressor(input_image, referframe)  

        semantic_feature, loss = self.srm(semantic_feature, H_f)

        resultFrame = self.videodecompressor(referframe, semantic_feature, semantic_feature_shape) 
