...
class Semantic_encoder(nn.Module):
    def __init__(self, in_channels=5):
        self.out_channel_N = 15  
        self.out_channel_M = 15
...
class VideoCompressor(nn.Module):
    ...
    def __init__(self):
        ...
        self.semantic_encoder = Semantic_encoder(5) 
    ...
    def forward(self, input_image, referframe):
        ...
        return semantic_feature, semantic_feature_shape
...
class FrameCodec(nn.Module):
    """
    :brief: Frame codec with semantic remapping module
    """
    def __init__(self, batch):
        super(FrameCodec, self).__init__()
        self.videocompressor = VideoCompressor()

        self.srm = Semantic_Remapping(batch)

        self.videodecompressor = VideoDecompressor()

    def forward(self, referframe, input_image, H_f):
        input_image = input_image/255.
        referframe = referframe/255.
        semantic_feature, semantic_feature_shape = self.videocompressor(input_image, referframe)  # encoding

        semantic_feature, loss = self.srm(semantic_feature, H_f)  # srm

        resultFrame = self.videodecompressor(referframe, semantic_feature, semantic_feature_shape)  # decoding
        resultFrame = resultFrame*255

        return resultFrame, loss


if __name__ == "__main__":
    model = FrameCodec(1).to(device)

    model.eval()
    with torch.no_grad():
        ref_frame = torch.randint(0, 255, (1, 3, 240, 320)).to(device)
        input_frame = torch.randint(0, 255, (1, 3, 240, 320)).to(device)
        H_f = np.random.random((64, 2, 2))
        result = model(ref_frame, input_frame, H_f)[0]
    print(result.shape)
