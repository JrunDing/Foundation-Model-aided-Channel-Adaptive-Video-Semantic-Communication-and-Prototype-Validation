...
if __name__ == "__main__":
    ...
    metrics = Metrics()
    videoCodec = FrameCodec().to(device)

    videoCodec.videocompressor.load_state_dict(
        torch.load('../checkpoints/Direct/9000/C_epoch87_loss39.09.pth'))
    videoCodec.videodecompressor.load_state_dict(
        torch.load('../checkpoints/Direct/9000/DC_epoch87_loss39.09.pth'))
        ...
        for j in range(len(videoPath[i])):
            ...
            if srcCapture.isOpened():
                for frame_idx_tx in range(srcFrameNum):
                    ...
                    with torch.no_grad():
                        if frame_idx_tx % GoP == 0:  
                            bits, bits_shape = videoCodec.videocompressor((imgSrc / 255.).to(device), (
            ...
            mimo_ofdm = MIMO_OFDM(2, 2, 64, "Perfect", "ZF",
                                  moduType='QPSK', channel_model="UMi")  
                ...
                for frame_idx_rx in range(srcFrameNum):
                    ...
                    with torch.no_grad():
                        if frame_idx_rx % GoP == 0: 
                            frame_tensor = videoCodec.videodecompressor(I_frame_, rxBits_PerFrame, bits_shape)
                            last_frame_rx = frame_tensor
                        else:  
                            frame_tensor = videoCodec.videodecompressor(last_frame_rx, rxBits_PerFrame, bits_shape)
                            last_frame_rx = frame_tensor
