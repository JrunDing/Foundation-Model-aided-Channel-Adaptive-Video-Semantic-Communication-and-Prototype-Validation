"""
@Brief: Test the performance of one frame  ROI
"""
import torch
...
def run_one_frame():
    ...
    @Brief: Test the performance of one frame  ROI
    """
    test_data = UCF101(path="../data/UCF101/UCF-101-TRA-S-OBJECT-IMG-1/")
    test_data_loader = DataLoader(test_data, batch_size=1, shuffle=True, num_workers=0, drop_last=True)

    frameCodec_model = FrameCodec_OB().to(device)
    frameCodec_model.videocompressor.load_state_dict(torch.load('../checkpoints/OBJECT/***.pth'))
    frameCodec_model.videodecompressor.load_state_dict(torch.load('../checkpoints/OBJECT/***.pth'))
