"""
@Brief: MIMO OFDM for video transmission LMAVC.
"""
...
class MIMO_OFDM(object):
    ...
    def MIMO_OFDM_Simulation_Modulation(self, txBits, SNRdB, modulation_method):
        """
        :brief: MIMO-OFDM仿真  一个时隙/一个子帧传输，即一个导频OFDM和多数据OFDM
                传输过程 :: ::::::   :表示一个OFDM符号，前两个用于信道估计，后面用于传输数据
        :param txBits: 待传输的比特 一维numpy
        :param SNRdB: 信噪比
        :param modulation_method: 调制方式
        :return:
        """
        ...
        mu = self.m_map[modulation_method]  # 每个子载波对应bit

        # 全局变量
        global rxFlowDetected, W
        ############################################################
        # ① 传输信道估计OFDM符号
        # 以2×2MIMO为例，需要两组OFDM符号（每组OFDM符号为两个OFDM符号，因为2个发射天线）的原因：首先MIMO-OFDM信道估计的原理可以参考电子笔记，第一组OFDM符号用于估计
        # C1（第一个子载波）上的h11和h21、C2上的h12和h22、C3上的h11和h21等等，第二组OFDM符号用于估计C1上的h12和h22、C2上的h11和h21等等。
        # 相当于一共需要估计K×noRX×noTX个h，如果是64×2×2=256个h，因此需要noRX组OFDM符号，每组noTX个OFDM符号，共K×noRX×noTX个OFDM符号
        ...
        # 过信道
        rxBlockPilotOFDM, averAmp = self.channel(txBlockPilotOFDM, self.H_freq)  # MIMO 多径衰落  averAmp是因为发送前进行功率归一，这是幅度衰减系数
        rxBlockPilotOFDM = self.addNoise(rxBlockPilotOFDM, SNRdB)  # 加噪
        # 信道估计
        # H11Est, H21Est, H12Est, H22Est = self.channelEstimation(self.CE, rxBlockPilotOFDM, averAmp)
        ...
        # 过信道
        rxBlockPilotOFDM_2, averAmp_2 = self.channel(txBlockPilotOFDM_2, self.H_freq)  # MIMO 信道增益
        rxBlockPilotOFDM_2 = self.addNoise(rxBlockPilotOFDM_2, SNRdB)  # 加噪
        ...
        ############################################################
        # ② 传输数据OFDM符号
        noBitsOnce = mu*self.K*self.noTX  # 一次传输的比特数  mu×64×2=m×128
        if len(txBits) % noBitsOnce:
            supNoBits = noBitsOnce - (len(txBits) % noBitsOnce)  # 补充的比特数
            txBits = np.concatenate((txBits, np.zeros(supNoBits, dtype=np.int32)))  # 速率匹配
        else:
            supNoBits = 0
        txSymbols = modulator.modulate(txBits)
        noTransmit = int(len(txSymbols) / (self.K*self.noTX))  # 传输的次数
        rxSymbols = np.array([], dtype=np.complex128)
        # 传输
        for i in range(noTransmit):
            txSymbols_ = txSymbols[i*(self.K*self.noTX):(i+1)*(self.K*self.noTX)]
            ...
            # 信道
            rxFlow, averAmp = self.channel(txFlow, self.H_freq)  # MIMO 信道增益
            rxFlow_ = self.addNoise(rxFlow, SNRdB)  # 加噪
            # 信号检测
            if self.SD == "ZF":
                W = self.genWZf(H11Est, H21Est, H12Est, H22Est)
            ...
            rxFlowDetected = rxFlowDetected*averAmp  # 放大信号
            rxSymbols_ = self.layerMerge(rxFlowDetected)
            rxSymbols = np.concatenate((rxSymbols, rxSymbols_))
