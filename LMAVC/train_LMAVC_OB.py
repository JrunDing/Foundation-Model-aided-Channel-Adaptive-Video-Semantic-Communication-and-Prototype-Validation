...
def main(args):
    train_data = UCF101(path="../data/UCF101/UCF-101-TRA-S-OBJECT-IMG-1/")
    valid_data = UCF101(path="../data/UCF101/UCF101-VAL-S-OBJECT-IMG-1/")
    ...
    frameCodec_OB_model = FrameCodec_OB().to(device)
    loss_fn = torch.nn.MSELoss(reduction='mean').to(device)  
    adam_optimizer = torch.optim.Adam(frameCodec_OB_model.parameters(), lr=1e-4, betas=(0.9, 0.99))
    scheduler = torch.optim.lr_scheduler.MultiStepLR(adam_optimizer, milestones=[40, 80], gamma=0.3)
    ...
    for epoch in range(epochs):
        for train_data_ in train_data_loader:
            ...
            frameCodec_OB_model.train()
            recovered_frame = frameCodec_OB_model(ref, frame) 

            loss = loss_fn(frame, recovered_frame) 
                ...
                with torch.no_grad():  
                    ...
                    for valid_data_ in valid_data_loader:
                        if valid_count < 5:
                            ...
                            recovered_frame_valid = frameCodec_OB_model(ref_valid, frame_valid)  
        ...
        scheduler.step()  
        total_train_step = 0
    torch.save(frameCodec_OB_model.videocompressor.state_dict(),
               "../checkpoints/C_epoch{}_loss{:.2f}.pth".format(epoch + 1, loss_per_epoch/step_count))
    torch.save(frameCodec_OB_model.videodecompressor.state_dict(),
